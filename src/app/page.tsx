...
export default async function HomePage() {
  const [company, categoryShelves] = await Promise.all([getCompany(), getHomeCategoryShelves()])
      ...
      <section className="py-16 brand-gradient-band">
        <div className="container-wide text-center">
          ...
          <Link href="/register" className="btn btn-accent text-lg px-8 py-3">
            Create an Account <ArrowRight className="w-5 h-5 ml-2 inline-block align-text-bottom" />
          </Link>
        </div>
      </section>
    </div>
